...
namespace OpenMQLib
{
	...
	public class OpenMQProxy
	{
		...
		public void CreateQueueDestination(OpenMQNative.MQHandle sessionHandle, string destinationName,
			ref OpenMQNative.MQHandle destinationHandle)
		{
			OpenMQNative.MQStatus res = OpenMQNative.MQCreateDestination(sessionHandle, destinationName,
							OpenMQNative.MQ_QUEUE_DESTINATION, ref destinationHandle);
			if (OpenMQNative.MQ_ERR_CHK(res))
				throw new OpenMQException("Error creating queue destination " + res.errorCode);
		}
